#!/usr/bin/env bash
# Verification harness for the Problem 20 formalization.
#
# Project layout (mirrors ~/dev/prob4bFORM):
#   * Sources live under Prob20/.
#   * The frozen pair Prob20/Defs.lean + Prob20/Theorems.lean are pinned by
#     SHA-256 in scripts/frozen.sha256.
#   * Theorems.lean holds the immutable statements as `sorry` stubs; the proofs
#     live in Prob20/Proofs/** and are exposed as clean, named theorems in
#     Prob20/Solution.lean (Prob20.Solution.<name>). Prob20/Discharge.lean
...
#   4. #print axioms        Each Prob20.Solution.<name> depends only on the
...
SRC_DIR="$REPO_ROOT/Prob20"
DEFS_FILE="$SRC_DIR/Defs.lean"
THEOREMS_FILE="$SRC_DIR/Theorems.lean"
PINS_FILE="$REPO_ROOT/scripts/frozen.sha256"

# The ten frozen theorems (= Prob20.Solution.<name> = Prob20.<name>).
ALL_THEOREMS=(D_residue_field D_fraction_field key_membership p_tp_linindep \
              theta2_kernel thetaN_not_injective g_mem theta2_missing \
              thetaN_not_surjective problem20_answer)
...
echo "=== Verifying Problem 20 formalization ==="
echo "  Target: $TARGET"
...
BANNED_OUT=$(SRC_DIR="$SRC_DIR" ROOT_LEAN="$REPO_ROOT/Prob20.lean" THEOREMS_FILE="$THEOREMS_FILE" python3 - <<'PY'
...
echo ""
echo "--- Check 4: #print axioms (Prob20.Solution.*) ---"
AX_FILE=$(mktemp /tmp/p20_ax_XXXX.lean)
trap 'rm -f "$AX_FILE"' EXIT
{ echo "import Prob20"; for t in "${TARGETS[@]}"; do echo "#print axioms Prob20.Solution.$t"; done; } > "$AX_FILE"
set +e
AX_OUTPUT=$(run_lake env lean "$AX_FILE" 2>&1)
set -e
AX_FAIL=0
for t in "${TARGETS[@]}"; do
    line=$(echo "$AX_OUTPUT" | grep "Prob20.Solution.$t' depends on axioms")
    if [ -z "$line" ]; then
...
GATE_FAIL=0
for mod in Prob20.Discharge Prob20.Solution; do
    set +e
